...
def main():
    ...
    for name, wx, wy, w in placed:
        ...
        A('    <!-- Коллизия рельсового ролика: ДВА цилиндра вместо меша.')
        A('         Профиль с канавкой невыпуклый, а движки быстро считают')
        A('         только выпуклые тела. Пара "дорожка + реборда" — это две')
        A('         выпуклые формы, вместе дающие ступеньку: рельса')
        A('         удерживается сбоку, но соскочить при перекосе можно,')
        A('         а это и есть предмет изучения на заезде.')
        A('         Чего пара не даёт — плавности галтели: переход выйдет')
        A('         ступенькой. Если заезд поведёт себя странно, смотреть сюда. -->')
        for rr_, (y0, y1), what in ((RAIL_TREAD_R, RAIL_TREAD_Y, 'дорожка'),
                                    (RAIL_FLANGE_R, RAIL_FLANGE_Y, 'реборда')):
            yc = rd * (y0 + y1) / 2
            A(f'    <collision><!-- {what} -->')
    ...
    A('    <parent link="base_link"/><child link="camera_link"/>')
    A('    <origin xyz="0.60 0 0.63"/>')
    A('  </joint>')
